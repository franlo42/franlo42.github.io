...
function updateCarHeight() {
  ...
  if (intersections.length > 0) {
    ...
    const clearance = 0.5; // separación deseada entre el coche y la pista
...
//Final del juego
const finishLineStart = new THREE.Vector3(-75, 13, 0);  // Punto A de la línea
const finishLineEnd = new THREE.Vector3(-75, 13, 20);  // Punto B de la línea
...
function checkFinishLineCrossing() {
  ...
  const carPosition = new THREE.Vector2(carMesh.position.x, carMesh.position.z);
  ...
  // Producto cruzado para determinar de qué lado de la línea está el coche
  const crossProduct = (end2D.x - start2D.x) * (carPosition.y - start2D.y) - 
                       (end2D.y - start2D.y) * (carPosition.x - start2D.x);

  // Determinar el lado del coche (positivo o negativo)
  const currentSide = crossProduct > 0 ? 1 : -1;

  // Si el coche cambia de lado, significa que ha cruzado la línea
  ...
  lastSide = currentSide; // Guardamos el lado actual para la próxima iteración
}
